# -*- coding: utf-8 -*-
"""
Created on Wed May 31 22:02:12 2023
...
def trajectory_optimization(x0, xf, path, obstacles): #initial state, final state, (Hybrid A*) path, obstacles list
    # Solver variables
    # Pick T and N so that we have around 0.1s dt, for this we need to estimate T (total time duration of path)
    # from there we can pick N
...
# Test the function
N = 120 # SAME AS FUNCTION BE SURE TO CHECK

# Initial and final conditions
x0 = np.array([50, 0, 0, 0, 0])
xf = np.array([170, 100, 0, 0, 0])
...
x_path = np.concatenate((np.linspace(x0[0], 100, N//3), np.linspace(100, 120, N//3), np.linspace(120, xf[0], N//3 + 1)))

y_path = np.concatenate((np.linspace(x0[1], 40, N//3), np.linspace(40, -40, N//3), np.linspace(-40, xf[1], N//3 + 1)))

theta_path = np.concatenate((np.full((N//3,), theta_init), np.full((N//3,), -theta_init), np.full((N//3 + 1,), theta_init)))

path = np.array([x_path, y_path, theta_path, np.zeros(N+1), np.zeros(N+1)])


X_sol, U_sol = trajectory_optimization(x0, xf, path, obstacles)
